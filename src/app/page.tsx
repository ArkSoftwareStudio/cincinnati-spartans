...
const footerNavigation: FooterCategories[] = [
  {
    name: 'About',
    links: [
      { name: 'Organization', href: '/about/#us' },
  ...
  },
  {
    name: 'Teams',
    links: [
      { name: 'Overwatch', href: '/teams/overwatch' },
      { name: 'Rainbow Six Siege', href: '/teams/rss' },
...
var navLinkClass = 'text-xs font-chakra drop-shadow-lg lg:text-lg lg:font-medium bg-blur text-slate-100 drop-shadow-sm hover:scale-[1.2] transition-all'
const staffMembers = [
  {
    'name': 'Ayden Springer',
    'nickname': 'Kryptic',
...
export default function Home() {
  return (
    <main className="flex min-h-screen grow w-full lg:w-[95vw] border border-t-0 border-b-0 border-sky-500/50 mx-auto flex-col items-center justify-between">
      <section className="relative z-50 border-b border-sky-500/50 flex flex-col h-[80vh] w-full bg-zinc-900/10">

        {/*  Navigation */}
        <div className="absolute top-0 w-full h-full">
          <nav className="sticky backdrop-blur-sm z-40 max-w-fit h-16 lg:inset-x-0 lg:mx-auto max-h-12 shadow-md shadow-slate-950/40  bg-zinc-950  bg-blur flex space-x-4 px-6 items-center justify-between top-0 lg:top-14 lg:my-14">
            <div className="flex justify-between space-x-8 p-2">
              <Link href='/' className={navLinkClass}>Home</Link>
              <Link href='/staff' className={navLinkClass}>Staff</Link>
            </div>

            <div className="overflow-visible mx-2 invisible md:visible">
      ...
      <section className="relative grow z-50 border-b border-sky-500/50 flex flex-col flex-wrap w-full h-[230vh] backdrop-brightness-50">

        <div className="w-full h-48 lg:h-80 select-none border-b border-sky-500/50 flex flex-row justify-center flex-wrap">
          <div className="relative w-full overflow-hidden flex justify-center items-center">
            <div className="absolute h-full w-48 lg:w-80 rotate-45 border border-sky-500"></div>
            <div className="absolute invisible md:visible left-14 top-0 h-1/2 md:w-1/5 lg:w-1/4 border-b border-sky-500"></div>
            <div className="absolute invisible md:visible right-14 top-0 h-1/2 md:w-1/4 lg:w-1/4 border-b border-sky-500"></div>
            <ChevronDoubleLeftIcon className="absolute w-4 lg:w-8 stroke-sky-500 left-8" />
            <ChevronDoubleRightIcon className="absolute w-4 lg:w-8 stroke-sky-500 right-8" />
            <div className="flex flex-col justify-center items-center">
              <h1 className="font-chakra h-2 lg:h-4 text-xs lg:text-lg font-bold text-sky-400 uppercase">Where Legends are Born</h1>
              <p className="font-chakra mt-2 h-4 text-xs lg:text-sm font-light text-white uppercase">Meet the Teams</p>
              <a href="#Rosters" className="z-20 transition-all duration-500 flex flex-row justify-center items-center text-black bg-sky-500 hover:bg-sky-600 font-chakra font-medium px-6 py-2 my-2 lg:text-2xl">Rosters <ArrowUpRightIcon className="w-4 lg:w-6 p-1 stroke-black" /> </a>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[url('imgs/endless-clouds.svg')] border-b border-sky-500/50 flex flex-col lg:flex-row bg-zinc-950 w-full h-[90vh]">
          {/* Left Side */}
          <div className="w-full lg:w-1/2 flex flex-col h-full border-r border-sky-500/50">
            <div className="w-full border-b border-sky-500/50 h-1/2 flex flex-row">
              {socials.map((social, index) => (
                <div className={`relative lg:h-full w-1/3 flex justify-center items-center text-zinc-700 ${index === 1 ? 'border-x border-sky-500/50' : ''}`} key={index}>
                  ...
                  <a target="_blank" className="h-full w-full flex justify-center items-center hover:bg-zinc-800/50" href={social.link}>
                    <FontAwesomeIcon className='text-sky-700 max-w-24 w-14 my-4 lg:w-24 text-2xl' icon={social.icon} />
                  </a>
                </div>
              ))
          ...
          {/* Right Side */}
          <div className="w-full lg:w-1/2 h-full border-sky-500/50 flex flex-col">

            {/* Upper Container */}
            <div className=" lg:h-1/2 w-full grow flex flex-col justify-center items-center px-8 border-b border-sky-500/50">
              <h1 className="italic font-chakra font-light text-xs my-2 lg:text-xl text-center text-sky-500">
            ...
            <div className="h-1/2 w-full flex flex-row">
              <div className="h-full w-1/3 bg-zinc-950/90 hover:bg-zinc-900/90 flex justify-center items-center">
                <Image className="w-16 lg:w-44" src={ValorantLogo} alt="Valorant Logo"></Image>
              </div>
              <div className="h-full w-1/3 bg-zinc-950/90 hover:bg-zinc-900/90 border-x border-sky-500/50 flex justify-center items-center">
                <Image className="w-10 lg:w-24" src={RainbowSix} alt="Rainbow Six Logo"></Image>
              </div>
              <div className="h-full w-1/3 bg-zinc-950/90 hover:bg-zinc-900/90 flex justify-center items-center">
                <Image className="w-20 lg:w-56" src={Overwatch} alt="Overwatch 2 Logo"></Image>
              </div>
            </div>
      ...
      <footer className="relative h-max flex flex-col justify-between lg:flex-row py-10 px-8 w-full bg-black text-zinc-500 font-chakra">
        <div className="order-last lg:order-first w-full lg:w-1/3 justify-start flex flex-col">
          <Image className="h-20 w-20 self-center lg:self-start lg:h-28 lg:w-28" src={csLogo} alt="CS LOGO"></Image>
          <h1 className="ml-4 text-sm"><span>&copy;</span>2024. Cincinnati Spartans. All Rights Reserved</h1>
          <div className="lg:ml-4 w-full mt-4 flex flex-row justify-between lg:w-1/4 text-sky-600 text-xl">
            <a><FontAwesomeIcon className="text-zinc-500 mx-1 h-6" icon={faDiscord} /></a>
            <a><FontAwesomeIcon className="text-zinc-500 mx-1 h-6" icon={faInstagram} /></a>
            <a><FontAwesomeIcon className="text-zinc-500 mx-1 h-6" icon={faTwitch} /></a>
            <a><FontAwesomeIcon className="text-zinc-500 mx-1 h-6" icon={faXTwitter} /></a>
          </div>
        </div>

        <div className="w-full lg:w-1/3 uppercase flex flex-row items-start">
          {
            footerNavigation.map((cat, index) => (
              <div className="w-full lg:w-1/2 uppercase flex flex-col" key={index}>
                <h1 className="text-xs lg:text-xl font-medium mb-2 text-zinc-200">{cat.name}</h1>
                {cat.links.map((link, idx) => (
                  <Link className="text-xs lg:text-xl my-1 w-fit hover:text-zinc-400" key={idx} href={link.href}>{link.name}</Link>
                ))}
              </div>
            ))
          }
        ...
        <div className="h-full mt-4 flex flex-col justify-between items-center">
          <Image className="w-full" src={Razer} alt="Razer Logo"></Image>
          <Image className="w-full" src={Rogue} alt="Rogue Logo"></Image>
        </div>
      </footer>
