...
export default function Home() {
  return (
    <main className="flex min-h-screen grow w-[95vw] border border-t-0 border-b-0 border-sky-500/50 mx-auto flex-col items-center justify-between">
      ...
      <section className="relative grow z-50 border-b border-sky-500/50 flex flex-col flex-wrap w-full h-[230vh] backdrop-brightness-50">
        ...
        <div className="w-full h-80 select-none border-b border-sky-500/50 flex flex-row justify-center flex-wrap">
          <div className="relative w-full overflow-hidden flex justify-center items-center">
            ...
            <div className="absolute left-14 top-0 h-1/2 md:w-1/5 lg:w-1/4 border-b border-sky-500"></div>
            <div className="absolute right-14 top-0 h-1/2 md:w-1/4 lg:w-1/4 border-b border-sky-500"></div>
            ...
            <div className="flex flex-col justify-center items-center">
              <h1 className="font-chakra h-4 text-lg font-bold text-sky-400 uppercase">Where Legends are Born</h1>
              <p className="font-chakra mt-2 h-4 text-sm font-light text-white uppercase">Meet the Teams</p>
      ...
      <footer className="relative h-max flex flex-row py-10 px-8 w-full bg-black text-zinc-500 font-chakra">
        <div className="w-1/3 justify-start flex flex-col">
        ...
        <div className="w-1/3 uppercase flex flex-row items-start">
          {
            footerNavigation.map((cat, index) => (
              <div className="w-1/2 uppercase flex flex-col" key={index}>
                <h1 className="font-medium mb-2 text-zinc-200">{cat.name}</h1>
                {cat.links.map((link, idx) => (
                  <Link className="my-1 w-fit hover:text-zinc-400" key={idx} href={link.href}>{link.name}</Link>
                ))}
              </div>
            ))
          }
        ...
        <div className="h-full flex flex-col justify-between items-center">
          <Image height={50} src={Razer} alt="Razer Logo"></Image>
          <Image height={180} src={Rogue} alt="Rogue Logo"></Image>
        </div>
      </footer>
